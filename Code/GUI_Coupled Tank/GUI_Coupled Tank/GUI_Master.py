...
class ComGUI():
    def __init__(self, root, serial):
        ...
        self.label_com = Label(
            self.frame, text="Available Port(s): ", bg="white", width=10, anchor="w")
...
class Window_Auto():
    def __init__(self, root, serial):
        ...
        self.inletData = DoubleVar()                 # Data List untuk inlet flow
        self.Tank1Data = DoubleVar()                 # Data List untuk Tank 1
        self.Tank2Data = DoubleVar()                 # Data List untuk Tank 2
    ...
    def Connframe(self):
        ...
        self.pump_label = Label(self.frame_conn, text="Pump Status: ",
                                bg="white", width=15, anchor="w")
        
        self.btn_start = Button(self.frame_conn, text="Start", state="active",
                                width=7, command=self.start_pump)
        self.btn_stop = Button(self.frame_conn, text="Stop", state="active",
                               width=7, command=self.stop_pump)
    ...
    def Connframe_publish(self):
        ...
        self.window1.geometry("290x170")
        self.frame_conn.grid(row=0, column=0, rowspan=3,
                        columnspan=2, padx=5, pady=5)

        self.pump_label.grid(column=0, row=0)

        self.btn_start.grid(column=1, row=0, padx=self.padx)
        self.btn_stop.grid(column=1, row=1, padx=self.padx, pady=self.pady)

        self.save_check.grid(column=0, row=2)
    ...
    def data_to_csv(self):
        ...
        while self.save:
           ...
           # with open(self.filename, 'a', newline='') as file:
                #writer = csv.writer(file)
                #for i in range(len(self.dataList_Inflow)):
                    #data = [self.XData[i], self.dataList_Inflow[i], self.dataList_Tank1[i], self.dataList_Tank2[i]]
                    #writer.writerow(data)
    ...
    def Displayframe_publish(self):
        ...
        self.inletData_label.grid(column=1, row=0, pady=9, padx = 18, sticky="w")
        
        self.Tank1_label.grid(column=0, row=1)
        self.Tank1Data_label.grid(column=1, row=1, pady=9, padx = 18, sticky="w")

        self.Tank2_label.grid(column=0, row=2)
        self.Tank2Data_label.grid(column=1, row=2, pady=10, padx = 18, sticky="w")
    ...
    def update_data(self):
        while True:
            ...
            data_list = Data_string.split(',', maxsplit=4)
            if len(data_list) == 4:
    ...
    def send_data_to_arduino(self):
        ...
        dataTosend = "{}-{}-{}-{}".format(self.num_kp, self.num_ki, self.num_kd, self.num_setpoint)
    ...
    def animate3(self, i):
        self.ax3.clear()
        self.ax3.set_title("Inlet Flow", fontsize=10)
        ...
        x_time = [round(val, 2) for val in x]
        self.ax3.plot(x_time, y, color="red",dash_capstyle='projecting', linewidth=1)
    ...
    def AdjustData(self):
        ...
        if lenInflowData > 50 and lenTank1Data > 50 and lenTank2Data > 50:
...
class Window_Manual():
    ...
    def Connframe(self):
        self.frame_conn = LabelFrame(self.parent_frame, text="Main Manager",
                                padx=5, pady=5, bg="white", width=60)
    ...
    def Connframe_publish(self):
        ...
        self.save_check.grid(column=2, row=1, padx=4)
    ...
    def Motorframe(self):
        ...
        self.slider_valve = Scale(self.frame_motor, from_=0, to=3600, orient="horizontal", 
                                length=300, sliderlength=20, showvalue=0, tickinterval=800, resolution=400, command=self.update_label)
    ...
    def update_data(self):
        while True:
            ...
            data_list = Data_string.split(',', maxsplit=4)
            if len(data_list) == 4:
                _, _, inletData_split, valveData_split = data_list
                try:
    ...
    def send_data_to_arduino(self):
        print("Sending the data...")
        self.serial.SerialWrite(b'd')

        self.num_valve = self.slider_valve.get()

        dataTosend = "{}\n".format(self.num_valve)
        self.serial.SerialWriteEncode(dataTosend)
        time.sleep(0.1)

        # waiting response from Arduino Master
        #response = self.serial.SerialReadline()
        #if response.strip() == "r":
            #print("Data", dataTosend, "terkirim")
